...
APP_VERSION = "2.38.0-noexcel-v38-ui-usability"
...
class MultiPaneLogViewer(LegacyDualLogViewer):
    ...
    def default_viewer_columns_for_rows(self, rows: list[dict[str, Any]]) -> list[str]:
        # Keep message readability by default. Extra Review parameters are hidden
        # until the operator opens Columns....
        base = ["Timestamp", "Message", "Level"]
        return [c for c in base if any(c in r for r in rows[:50])] or base

    def all_available_columns_for_rows(self, rows: list[dict[str, Any]]) -> list[str]:
        preferred = ["Timestamp", "Message", "Level", "SourceType", "Category", "File", "Line"]
        found = []
        for c in preferred:
...
class MainWindow(QWidget):
    ...
    def build_ui(self):
        ...
        subtitle = QLabel("No Microsoft Excel / No VBA / No COM. v38: UI usability improvements + flexible Log Viewer layout.")
